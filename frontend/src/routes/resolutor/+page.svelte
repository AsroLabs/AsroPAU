<script lang="ts">
  ...
  import StepsSolver from "$features/math-solver/components/StepsSolver.svelte";
  ...
  function saveToHistory(input: string, res: SolveResult) {
    const entry: HistoryEntry = {
      ...
      ts: new Intl.DateTimeFormat(navigator.languages as string[], {
        hour: "2-digit",
        minute: "2-digit",
      }).format(new Date()),
    };
    history = [entry, ...history.filter((h) => h.input !== input)].slice(
      0,
      MAX_HISTORY,
    );
    try {
  ...
  function humanizeError(raw: string): string {
    ...
    if (raw.includes("antiderivada en forma cerrada")) return raw; // Already friendly
  ...
  const exampleGroups = [...new Set(examples.map((e) => e.category))].map(
    (cat) => ({
      cat,
...
<div
  class="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50/30 py-10 px-4"
>
  <div class="max-w-2xl mx-auto space-y-5">
    ...
    >
      ...
      <div class="relative z-10 flex items-start justify-between gap-4">
        <div>
          <h1 class="text-2xl font-extrabold mb-1 tracking-tight">
            Resolutor de Matemáticas
          </h1>
          <p class="text-orange-200 text-sm leading-relaxed">
            Ecuaciones, derivadas, integrales y límites — con solución paso a
            paso animada.
          </p>
        </div>
        ...
        <div class="p-3 flex flex-col gap-1.5 max-h-64 overflow-y-auto">
          {#each history as entry (entry.id)}
            ...
            >
              <div class="min-w-0">
                <p
                  class="font-mono text-sm truncate text-slate-700 group-hover:text-orange-700"
                >
                  {entry.input}
                </p>
              ...
              <span class="shrink-0 text-xs text-slate-400 ml-3"
                >{entry.ts}</span
              >
            </button>
          {/each}
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
      ...
      >
        <button
          ...
          class="px-4 py-1.5 rounded-lg text-sm font-semibold transition-all duration-150 cursor-pointer
            ...
            : 'text-slate-500 hover:text-slate-700'}">Texto / LaTeX</button
        >
        <button
          role="tab"
          ...
          class="px-4 py-1.5 rounded-lg text-sm font-semibold transition-all duration-150 cursor-pointer
            {activeMode === 'keyboard'
            ? 'bg-white shadow text-orange-600'
            : 'text-slate-500 hover:text-slate-700'}">Teclado</button
        >
      </div>
      ...
      <div class="mt-5 space-y-2.5">
        {#each exampleGroups as group}
          <div>
            <p
              class="text-xs text-slate-400 mb-1.5 font-semibold uppercase tracking-widest"
            >
              {group.cat}
            </p>
            <div class="flex flex-wrap gap-1.5">
              {#each group.items as ex}
                <button
                  ...
                  title="Cargar: {ex.val}">{ex.label}</button
                >
              {/each}
            </div>
          </div>
    ...
    {#if error}
      ...
      >
        ...
        <div>
          <p class="font-semibold text-red-700 mb-1 text-sm">
            Error al resolver
          </p>
          <p class="text-sm text-red-600">{error}</p>
          <p class="text-xs text-red-400 mt-2">
            Sintaxis: usa <code class="font-mono bg-red-100 px-1 rounded"
              >^</code
            >
            o <code class="font-mono bg-red-100 px-1 rounded">**</code> para
            potencias,
            <code class="font-mono bg-red-100 px-1 rounded">*</code> para
            multiplicar. Ej:
