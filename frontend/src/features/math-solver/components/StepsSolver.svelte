<script lang="ts">
  ...
  // ── Props ─────────────────────────────────────────────────────────────────
  let { steps = [], result = null }: { steps: Step[]; result: Result | null } =
    $props();
  ...
  function revealStep(index: number) {
    ...
    if (autoMode) {
      const totalStepMs =
        STEP_ENTER_MS + STEP_HIGHLIGHT_MS + STEP_EXPLAIN_MS + STEP_GAP_MS;
      timers.push(
        setTimeout(() => {
          highlightIndex = -1;
          revealStep(index + 1);
...
>
  ...
  {#if phase !== "idle"}
    ...
    >
      <div class="flex items-center gap-3">
        ...
        {#if result}
          <span
            ...
            class="text-xs font-semibold px-2.5 py-1 rounded-full text-white {typeColor(
              result.type,
      ...
      <div class="flex items-center gap-2">
        <span class="text-xs text-gray-400 dark:text-slate-500 hidden sm:inline"
          >Ritmo:</span
        >
        <div
          class="flex gap-0.5 bg-gray-100 dark:bg-slate-700 p-0.5 rounded-lg"
        >
          <button
            onclick={() => {
              autoMode = true;
  ...
  <!-- ── Steps area ─────────────────────────────────────────────────────── -->
  <div class="p-5" aria-live="polite" aria-atomic="false">
        ...
        {#each steps.slice(0, visibleCount) as step, i (step.step_number)}
          ...
          <div
            ...
            style:box-shadow={isActive
              ? `0 0 0 3px ${hColor}22, 0 4px 16px ${hColor}18`
              : undefined}
            ...
            <div class="flex items-start gap-3 p-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-start justify-between gap-2 mb-2">
                  <p
                    class="text-sm font-semibold text-gray-700 dark:text-slate-200 leading-snug"
                  >
                    {step.description}
                  </p>
                  {#if step.rule_name}
                ...
                <div
                  class="text-lg rounded-xl bg-white dark:bg-slate-900/40
                ...
                >
                  {#key isActive}
                    <div
                      in:scale={{
                        duration: isActive ? 240 : 160,
                        easing: backOut,
                        start: isActive ? 0.96 : 1,
                      }}
                      class="katex-highlight-wrap {isActive ? 'is-active' : ''}"
                    >
                      <MathRenderer
                        latex={isActive && step.highlighted_latex
                          ? step.highlighted_latex
                          : step.expr_latex}
                        inline={false}
                      />
                    </div>
                ...
                {#if step.explanation && (isComplete || isActive)}
                  ...
                  >
                    <svg
                      class="shrink-0 w-3.5 h-3.5 mt-0.5 text-gray-400 dark:text-slate-500"
                      fill="none"
                      viewBox="0 0 16 16"
                      stroke="currentColor"
                      stroke-width="1.5"
                    ...
                    <p
                      class="text-xs text-gray-500 dark:text-slate-400 leading-relaxed"
                    >
                      {step.explanation}
                    </p>
                  </div>
        ...
        {#if phase === "stepping" && visibleCount < steps.length}
          {#each steps.slice(visibleCount, visibleCount + 2) as _, i}
            ...
            >
              <div class="flex items-center gap-3">
                <div
                  class="w-7 h-7 rounded-full bg-gray-200 dark:bg-slate-700 animate-pulse shrink-0"
                ></div>
                <div class="flex-1 space-y-2">
                  <div
                    class="h-2.5 bg-gray-200 dark:bg-slate-700 rounded animate-pulse w-2/5"
                  ></div>
                  <div
                    class="h-10 bg-gray-100 dark:bg-slate-800 rounded-xl animate-pulse"
                  ></div>
                </div>
              </div>
            </div>
  ...
  {#if phase === "result" && result}
    ...
    >
      ...
      <!-- Result math -->
      <div
        class="bg-gradient-to-b from-orange-50 to-white dark:from-orange-950/30
                  dark:to-slate-800/50 px-5 py-5"
      >
        {#if result.type === "no_solution"}
          <p
            class="text-center text-orange-700 dark:text-orange-300 font-semibold text-sm"
          >
        ...
        {:else}
          ...
          <div class="text-2xl">
            <MathRenderer
              latex={result.latex ?? result.result ?? ""}
              inline={false}
            />
          </div>
        {/if}
          ...
          {#each result.solutions as sol, i}
            ...
            >
              <span class="opacity-70 text-xs">
                x{#if result.solutions && result.solutions.length > 1}<sub
                    >{i + 1}</sub
                  >{/if} =
              </span>
              {sol}
...
<style>
  ...
  */
  ...
  :global(
      .katex-highlight-wrap.is-active .katex [style*="color:"]:nth-child(2)
    ) {
    animation-delay: 0.06s;
  }
  :global(
      .katex-highlight-wrap.is-active .katex [style*="color:"]:nth-child(3)
    ) {
    animation-delay: 0.12s;
  }
  :global(
      .katex-highlight-wrap.is-active .katex [style*="color:"]:nth-child(4)
    ) {
